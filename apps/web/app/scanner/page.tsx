...
export default function ScannerPage() {
  ...
  useEffect(() => { const base = process.env.NEXT_PUBLIC_API_BASE_URL ?? "/api/v1"; fetch(`${base}/scanner?timeframe=5m`).then(r => r.ok ? r.json() : Promise.reject(r)).then(setRows).catch(() => setError(true)); }, []);
  ...
  return <main className="scanner-page">
    <div className="scanner-heading"><div><p className="eyebrow">MARKET INTELLIGENCE</p><h1>Futures scanner</h1><p className="lead">Confluence-only analysis for NQ, ES, YM, RTY and GC. Select a market to inspect its structure.</p></div><div className="data-badge" aria-label="Current data status">MOCK DATA · NO LIVE EXECUTION</div></div>
    {error ? <p className="empty">Scanner API unavailable. Check the active Web App deployment.</p> : rows === null ? <p className="empty">Loading scanner…</p> : <>
      <div className="market-tabs" role="tablist" aria-label="Markets">{rows.map(row => <button key={row.symbol} role="tab" aria-selected={selected?.symbol === row.symbol} className={selected?.symbol === row.symbol ? "market-tab selected" : "market-tab"} onClick={() => setSelectedSymbol(row.symbol)}>{row.symbol}<span>{row.bias.direction}</span></button>)}</div>
    ...
    <p className="disclaimer">MOCK is deterministic development data, not live quotes. Educational analysis only; no execution or guarantees.</p>
  </main>;
}

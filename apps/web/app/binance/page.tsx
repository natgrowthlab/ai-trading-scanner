...
export default function BinancePage() {
  ...
  return <main className="binance-page">
    <div className="scanner-heading"><div><p className="eyebrow">BINANCE USDT‑M FUTURES</p><h1>Binance Futures</h1><p className="lead">Contratos perpetuos USDT‑M actualizados cada cinco segundos. El panel identifica claramente si el origen disponible es Live o Testnet y no tiene permisos para ejecutar órdenes.</p></div><p className="live-badge"><span /> {market?.marketDataMode === "TESTNET" ? "TESTNET FALLBACK" : "LIVE / USDT‑M"}</p></div>
    ...
    <section className="binance-summary" aria-live="polite"><article className="price-card"><p className="detail-label">{symbol} PERPETUAL</p><strong>{market ? `$${money(market.price)}` : "—"}</strong><span className={changeClass}>{market ? `${market.changePercent24h >= 0 ? "+" : ""}${market.changePercent24h.toFixed(2)}% / 24h` : "Loading price…"}</span></article><article><p className="detail-label">Data source</p><strong>{market?.marketDataMode ?? "—"}</strong><p>{market?.marketDataMode === "TESTNET" ? "Official Futures Testnet fallback; prices are not live." : "Live Binance USDT‑M Futures data."}</p></article><article><p className="detail-label">24h volume</p><strong>{market ? `$${compact(market.quoteVolume24h)}` : "—"}</strong><p>Quote volume in USDT.</p></article><article><p className="detail-label">Order book spread</p><strong>{market ? `$${money(market.orderBook.spread)}` : "—"}</strong><p>{market ? `Bid pressure ${(market.orderBook.imbalance * 100).toFixed(0)}%` : "Loading…"}</p></article></section>
    ...
    <p className="disclaimer">Datos de Binance USDT‑M Futures para análisis. Los futuros de cripto son volátiles; no hay garantías de rendimiento ni ejecución automática desde este panel.</p>
  </main>;
}

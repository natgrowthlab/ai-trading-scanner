...
input double          InpImpulseATRMultiple = 0.30;
input double          InpRejectionWickBodyRatio = 0.20;
...
void EvaluateEntry(const bool intrabar=false)
{
   ...
   else if(!intrabar && InpUseConfirmedTrendPullback)
   {
      ...
      double pullbackBuffer=InpRetracementBufferPoints*_Point;
      bool bullishRejection=rates[signalShift].low<=entryEma+pullbackBuffer && rates[signalShift].close>entryEma && rates[signalShift].close>rates[signalShift].open && lowerWick>=signalBody*InpRejectionWickBodyRatio;
      bool bearishRejection=rates[signalShift].high>=entryEma-pullbackBuffer && rates[signalShift].close<entryEma && rates[signalShift].close<rates[signalShift].open && upperWick>=signalBody*InpRejectionWickBodyRatio;
